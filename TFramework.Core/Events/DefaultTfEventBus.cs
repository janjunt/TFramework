...
using TFramework.Core.Exceptions;
...
namespace TFramework.Core.Events
{
    public class DefaultTfEventBus : IEventBus
    {
        ...
        private IEnumerable<object> NotifyHandlers(string messageName, IDictionary<string, object> eventData)
        {
            ...
            {
                throw new ArgumentException(T("{0}格式不正确", messageName).Text);
            }
        ...
        {
            var matchingInterface = eventHandler.GetType().GetTypeInfo().GetInterface(interfaceName);
